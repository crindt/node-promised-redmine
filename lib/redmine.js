...
var proto = {
	...
	, request: function(method, path, params){
		...
		req = (self.protocol==='https' ? https : http).request(options, function(res){
			...
			res.on('end', function() {
				if( !~([200,201]).indexOf(res.statusCode) ){
					...
					d.reject('STATUSCODE_REJECTION ' + res.statusCode+":"+body);
					return;
				}
				try {
		...
		if (method == 'POST' && params.body !== undefined) {
			// If POST and body is defined, use that (along with optional content-type)
			var body = params.body;
			req.setHeader('Content-Length', body.length);
			req.setHeader('Content-Type', params.contentType || 'application/json');
			req.write(body);
		} else if (method !== 'GET') {
